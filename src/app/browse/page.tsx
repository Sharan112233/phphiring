...
export default function BrowsePage() {
    ...
    const fetchTalent = async () => {
      try {
        ...
        if (res.ok) {
          ...
          console.error('Failed to fetch talent')
        }
      } catch (err) {
  ...
  useEffect(() => {
    ...
    // Skill filters (any skill selected)
    const allSelected = [
        ...
        <div style={{
          ...
          <div style={{ marginBottom: 16 }}>
            ...
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
              {['All', 'available', 'part_time', 'unavailable'].map(a => (
                <button
                  key={a}
          ...
          <FilterGroup
            label="Frameworks"
            items={FRAMEWORKS}
            selected={selFrameworks}
            onToggle={v => toggle(selFrameworks, setSelFrameworks, v)}
          />
          ...
          <FilterGroup
            ...
            items={CMS_ECOM}
            selected={selCMS}
            onToggle={v => toggle(selCMS, setSelCMS, v)}
          />
          ...
          <FilterGroup
            ...
            items={CRMS}
            selected={selCRMs}
            onToggle={v => toggle(selCRMs, setSelCRMs, v)}
          />
          ...
          <FilterGroup
            ...
            items={COMBINED}
            selected={selCombined}
            onToggle={v => toggle(selCombined, setSelCombined, v)}
          />
          ...
          <FilterGroup
            ...
            onToggle={v => toggle(selVersions, setSelVersions, v)}
          />
          ...
          <FilterGroup
            ...
            onToggle={v => toggle(selLanguages, setSelLanguages, v)}
          />
        </div>
        ...
        <div>
          ...
          {filteredTalent.length === 0 ? (
            <div style={{
              ...
              <div style={{ fontSize: 48, marginBottom: 14, opacity: 0.35 }}>
                🔍
              </div>
              <h3 style={{
                fontSize: 18,
